...
interface EnginePipelineProps {
  ...
  onRunStep: (id: PipelineStepId) => Promise<string[]>;
  isRunning: boolean;
}
...
export default function EnginePipeline({
  ...
  const [pipelineLogs, setPipelineLogs] = useState<string[]>([
    ...
    "[SYSTEM] Loaded GÜZELCE 467 ADA 9 PARSEL blueprint.",
  ]);
  ...
  const runFullPipeline = async () => {
    ...
    for (const step of steps) {
      ...
      const logs = await onRunStep(step.id);
      setPipelineLogs((l) => [
        ...l,
        ...logs.map(log => `[${step.title.toUpperCase()}] ${log}`),
  ...
  const runSingleStep = async (stepId: PipelineStepId) => {
    ...
    const logs = await onRunStep(stepId);
    setPipelineLogs((l) => [
      ...l,
      ...logs.map(log => `[${step.title.toUpperCase()}] ${log}`),
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="flex flex-col h-full bg-zinc-900/80 border border-zinc-800 rounded-xl p-4 overflow-hidden">
        ...
        <div className="flex-1 font-mono text-[10px] text-zinc-400 space-y-1.5 overflow-y-auto max-h-[360px] pr-2">
