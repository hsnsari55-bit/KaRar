...
export default function RoadmapPanel() {
  ...
  // 1. Geometry Engine, 2. Topology Engine, 3. Canonical BIM Model, 4. Semantic Enrichment, 5. Blender 3D, 6. Desktop UI, 7. Cloud
  const phases: Phase[] = [
    {
      id: "phase1",
      title: "Faz 1 - Çekirdek Motor (Kritik Odak)",
      subtitle: "Geometry, Topology & Canonical BIM Core",
      status: "completed",
      techStack: ["Python", "ezdxf", "Shapely", "Math Algorithms"],
      description: "KaRar'ın kalbidir. 2D CAD verisini girdi alıp, temizler, onarır ve kusursuz topolojik ağı (snapping/polygonization) kurar. Çekirdek kod, arayüzden tamamen bağımsız (decoupled) bir backend servisi olarak çalışır ve tüm sistemin ortak olarak okuyacağı 'Canonical BIM Model' sözleşmesini üretir.",
      tasks: [
        { name: "Geometry Normalization: AutoCAD koordinat ölçekleme, temizleme ve onarım (Repair)", completed: true },
        { name: "Topology Engine: Çakışan çizgileri birleştirme (Snapping) ve oda poligonlarını çıkarma", completed: true },
        { name: "Canonical BIM Model: Tüm alt sistemlerin okuyacağı tek, resmi ve doğrulanabilir JSON şeması sözleşmesi", completed: true },
        { name: "Semantic Enrichment: Topolojik veriye göre odaları (Mutfak, Salon vb.) ve elemanları zenginleştirme", completed: true },
        { name: "Validation Engine: Modelin geometrik, topolojik ve mimari bütünlük standartlarının doğrulanması", completed: true }
      ]
    },
    {
      id: "phase2",
      title: "Faz 2 - Blender 3D Generator & IFC",
      subtitle: "Katı Model Üretimi ve BIM Standartları",
      status: "completed",
      techStack: ["Python", "IfcOpenShell", "Blender API", "GLB/OBJ"],
      description: "Doğruluğu %100 tescillenmiş Canonical BIM JSON verisinden yola çıkarak, endüstri standardı IFC dosyalarını ve web/mobil görüntüleyiciler için optimize edilmiş 3D mesh (GLB/GLTF) modellerini üretir. Hatalı geometrinin hatalı BIM üretmesini önlemek amacıyla, bu katman ancak Faz 1 (Geometri + Topoloji + Canonical Model) tam doğrulukla bittiğinde devreye girer.",
      tasks: [
        { name: "IfcOpenShell Entegrasyonu: Standart IFC şemasına parametrik veri yazımı", completed: true },
        { name: "Blender Parametrik Extrusion: Python Blender API ile otomatik katı B-Rep modelleme", completed: true },
        { name: "Mesh Optimizasyonu: Tarayıcı performansı için GLB/OBJ geometrisini sadeleştirme", completed: true }
      ]
    },
    {
      id: "phase3",
      title: "Faz 3 - Masaüstü Kullanıcı Arayüzü",
      subtitle: "Tauri / Electron Entegrasyonu",
      status: "completed",
      techStack: ["React", "Tauri", "Three.js / WebGL", "TailwindCSS"],
      description: "Son kullanıcının projelerini göreceği, çizgileri tutup sürükleyerek parametrik manuel müdahalelerde bulunabileceği yerel istemci katmanı. Tauri ile internetten bağımsız, yerel dosya sistemine doğrudan erişebilen ve kullanıcının GPU'sunu kullanan sıfır bulut maliyetli istemci hedeflenir.",
      tasks: [
        { name: "Tauri Entegrasyonu: Yerel dosya sistemi (AutoCAD/DXF) doğrudan okuma/yazma", completed: true },
        { name: "Parametrik 2D/3D Canvas: Çizgilere interaktif müdahale edilebilen WebGL editörü", completed: true },
        { name: "Çevrimdışı Çalışma Kabiliyeti: İnternet bağımsız yerel veri işleme motoru", completed: true }
      ]
    },
    {
      id: "phase4",
      title: "Faz 4 - Bulut & İş Birliği Özellikleri (Planlanan)",
      subtitle: "Proje Senkronizasyonu ve Ekip Paylaşımı",
      status: "planned",
      techStack: ["FastAPI", "WebSockets", "PostgreSQL / Firebase"],
      description: "Uygulamanın gelişimini tamamlamasından sonra devreye alınacak katman. Projelerin çoklu kullanıcıyla ortak çalışılması, sürüm geçmişi takibi ve tasarımsal kalite kontrolü yapan arka plan servislerinin bulut üzerinden koşturulması aşamasıdır.",
      tasks: [
        { name: "Gerçek Zamanlı Senkronizasyon: WebSockets ile mimar-mühendis anlık iş birliği", completed: false },
        { name: "Bulut Yedekleme: Projelerin versiyon geçmişiyle güvenli saklanması", completed: false },
        { name: "AI Kalite Kontrolü: Mekansal standartlara ve yönetmeliğe uygunluk testleri", completed: false }
      ]
    }
  ];
