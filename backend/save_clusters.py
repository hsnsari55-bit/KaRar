...
# walls.json oku
with open(
    r"C:\KaRar\outputs\walls_clean.json",
    "r",
    encoding="utf-8"
) as f:
    walls = json.load(f)
...
for wall in walls:
    # Debug: wall yapısını kontrol et
    if not wall:
        continue
    
    # walls'da start/end varsa (LINE)
    if "start" in wall and "end" in wall:
        x = (wall["start"][0] + wall["end"][0]) / 2
        y = (wall["start"][1] + wall["end"][1]) / 2
    
    # walls'da points varsa (LWPOLYLINE)
    elif "points" in wall and len(wall["points"]) > 0:
        ...
        # Eğer ne start/end ne points varsa skip et
        continue
...
if len(points) < 10:
    ...
    print("Villa kümeleri oluşturulamadı")
else:
    # DBSCAN
